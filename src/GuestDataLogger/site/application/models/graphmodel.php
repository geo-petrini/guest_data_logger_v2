<?php
	...
	class GraphModel{
        ...
        function fetchStands($proprietario, $filter){
			if($proprietario == ""){
				$query = "SELECT id, GROUP_CONCAT(DISTINCT(nome)) AS nome, GROUP_CONCAT(DISTINCT(luogo)) AS luogo, MIN(data_inizio) AS data_inizio, MAX(data_fine) AS data_fine, proprietario FROM stand WHERE isPublic = 1 GROUP BY $filter";
			}else{
				$query = "SELECT id, GROUP_CONCAT(DISTINCT(nome)) AS nome, GROUP_CONCAT(DISTINCT(luogo)) AS luogo, MIN(data_inizio) AS data_inizio, MAX(data_fine) AS data_fine, proprietario FROM stand WHERE proprietario = '$proprietario' OR isPublic = 1 GROUP BY $filter";
			}
		...
		function fetchDataset($stand_id, $num_webcam){
			$camp = $this->camp;
			$query = "SELECT SUM(numero_persone) AS numero_persone FROM stat WHERE stand_id = $stand_id AND num_webcam = $num_webcam GROUP BY $camp ORDER BY $camp";
		...
		function fetchDatasetTable($stand_id){
			$camp = $this->camp;
			$query = "SELECT $camp AS 'data', SUM(numero_persone) AS numero_persone, num_webcam FROM stat WHERE stand_id = $stand_id GROUP BY $camp, num_webcam ORDER BY num_webcam, $camp";
			$dataset = DatabaseModel::executeSelectQuery($query);
			return $dataset;
		}
	}
