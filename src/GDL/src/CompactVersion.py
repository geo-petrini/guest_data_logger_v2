...
class StartWindow():
    ...
    def testConnection(self, api_key):
        ...
        if response.status_code == 404: #200: 
            return True
        else:
            return False
...
class CameraWindow():
    ...
    def startFaceRecognition(self, capture=0):
        ...
        self.threadweb.daemon = True
        #self.threadweb._stop_event = thread.Event()
    ...
    def update_picture(self, frame):
        ...
        self.label.configure(image = finalFrame)    #TODO usare un canvas
...
class CameraRunner(thread.Thread):
...
if __name__ == '__main__':
    ...
    q =  Queue()
    sw = StartWindow(q) #si chiude da sola
    ...
    cw = CameraWindow(q, webcam_port=0, ex=ex)

    sd.join()

    print("Thread morte")
    while cw.threadweb.is_alive():
        time.sleep(0.5)  
        print(f" sd alive: {sd.is_alive()},cw alive: {cw.threadweb.is_alive()}")  
