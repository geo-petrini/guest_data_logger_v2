...
class SenderData (thread.Thread):
    ...
    def checkEx(self):
        try:
            if(self.ex.get(False)['exit']):
                ...
                print("INFO: exit from SenderData Thread")
                return False
            else:
    ...
    """
    def send_data(self):
        json_data = self.dataToJSON()
        if json_data is not None:
            url = str(config['site_sender'])
            req = requests.post(url, json= json_data, proxies=proxies)
            s = req.text
            logging.info(f'request text; {s}')
            print(f'INFO: request text; {s}')
        ...
        if len(api_key) == 0:
            ...
            if  cameraIndex != ["None"]:
                ...
                if self.testConnection(api_key) is True:
                    ...
                    print(f'INFO: entered on file Config.json')
...
class Capture(thread.Thread):
    ...
    def run(self):
        ...
        print(f'INFO: end while - exiting webcam procedure')
...
class CameraWindow():
    ...
    def _refresh(self, data_queue, stop_event):
        ...
        print(f'INFO: frame refresh started')
        exit_event = False
